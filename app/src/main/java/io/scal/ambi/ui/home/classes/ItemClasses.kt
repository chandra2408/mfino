...
open class ItemClasses : Parceble<ClassesData?> {
    ...
    @Expose
    internal var owners: List<String>? = null

    override fun parse(): ClassesData? {
        return parseAsClass()
    }

    fun parseAsClass(): ClassesData {
        return ClassesData(id!!,name,
                courseCode,
                ...
                admins,getMeetingDayandTime(meetingDayAndTimes!!),members,owners)
    }
    ...
    private fun getMeetingDayandTime(value: String): String {
        if (value.startsWith("{")) {
            ...
            if(meetingDayAndTimes.startHour!=null && meetingDayAndTimes.endHour!=null){
