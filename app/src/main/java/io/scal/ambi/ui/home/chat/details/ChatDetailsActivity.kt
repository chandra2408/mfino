...
class ChatDetailsActivity : BaseToolbarActivity<ChatDetailsViewModel, ActivityChatDetailsBinding>(), PickerViewController {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.cCreation!!.etMessage.addTextChangedListener(object : TextWatcher {
            ...
            override fun onTextChanged(p0: CharSequence?, p1: Int, p2: Int, p3: Int) {
                if(p0.isNullOrEmpty()){
                    binding.cCreation!!.ivSend.visibility = View.GONE
                    binding.cCreation!!.ivSendLike.visibility = View.VISIBLE
                    viewModel.isTyping.set(false)
                }else{
                    viewModel.isTyping.set(true)
                    binding.cCreation!!.ivSend.visibility = View.VISIBLE
                    binding.cCreation!!.ivSendLike.visibility = View.GONE
                }
            }
        })
    }
    ...
    private fun initRecyclerView() {
        ...
        binding.rvMessages.setEmptyView(binding.llEmptyView);
    ...
    private fun initToolbar() {
        currentToolbar = ToolbarType(IconImage(R.drawable.ic_back), Runnable { router.exit() },
                ChatDetailsTitleToolbarContent(viewModel), null, null)
    ...
    private fun initCreationListeners() {
        binding.pickerListener = object : PickerActionListener {
            override fun openCamera() = pickerViewModel.pickAnImage(this@ChatDetailsActivity,this@ChatDetailsActivity,true)
            override fun attachPicture() = pickerViewModel.pickAnImage(this@ChatDetailsActivity, this@ChatDetailsActivity,false)
                ...
                .subscribe {
                    ...
                    when (it) {
                        is ChatDetailsDataState.Data -> {
                            ...
                            var reversedList = it.allMessages.reversed()

                            //for alignment in chat view left or right
                            for (item in reversedList){
                                when(item){
                                    is UIChatMessage.TextMessage -> item.alignLeft = !item.sender.uid.equals(viewModel.currentUser.get().uid)
                                    is UIChatMessage.AttachmentMessage -> item.alignLeft = !item.sender.uid.equals(viewModel.currentUser.get().uid)
                                    is UIChatMessage.URLMessage -> item.alignLeft = !item.sender.uid.equals(viewModel.currentUser.get().uid)
                                    else -> {
                            ...
                            binding.rvMessages.smoothScrollToPosition(binding.rvMessages.getAdapter().getItemCount());
                    ...
                    val newCurrentToolbar =
                            when {
                                true == it.chatInfo?.canAddUsers && currentToolbar.rightIcon == null  ->
                                    ...
                                    currentToolbar.copy(rightIcon = IconImage(R.drawable.ic_active_add_user),
                                            rightIconClickListener = Runnable { viewModel.addOtherUsers() },
                                            rightIcon2 = IconImage(R.drawable.ic_video_call_append),
                                            rightIconClickListener2 = null)
