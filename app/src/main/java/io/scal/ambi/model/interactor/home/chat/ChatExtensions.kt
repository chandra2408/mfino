    ...
    return getUserProfile(message.sender, userRepository)
        .map { sender ->
            if (null == message.media) {
                ChatMessage.TextMessage(message.uid, message.index, sender, message.sendDate, message.message, emptyList(),null)
            } else {
                ChatMessage.AttachmentMessage(message.uid,
                                              ...
                                              message.sendDate,
                                              message.message,
                                              listOf(message.media.toAttachment()), emptyList(),null)
            }
        }
}