...
class PostsRepository @Inject constructor(private val postsApi: PostsApi,
    ...
    override fun getDiscussions(entityType: String, page: Long, entityId: String): Single<List<NewsFeedItem?>> {
        return postsApi.getDiscussion(entityType,page,entityId).map { it.parse() }
    }
    ...
    override fun postNewStatus(pinned: Boolean,
        ...
        userObject.addProperty("_id",asUserUid.uid)

        return postsApi
            .postNewStatus(
                StatusCreationRequest(userObject,
                                      pinned,
                                      locked,
                                      statusText,
                                      listOf(Host(PostHostKind.USER.toServerName(), asUserUid.uid)),
    ...
    override fun postNewAnnouncement(pinned: Boolean,
        ...
        var userObject = JsonObject()
        userObject.addProperty("_id",asUserUid)


        return postsApi
            .postNewAnnouncement(
                AnnouncementCreationRequest(userObject,
                                            pinned,
                                            locked,
                                            statusText,
