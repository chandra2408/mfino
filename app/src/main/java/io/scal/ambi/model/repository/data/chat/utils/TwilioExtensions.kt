    ...
    try {
        ...
        val media =
            when (messageType) {
                ...
                "HANDSHAKE"    -> ChatChannelMessage.Media(R.drawable.ic_thumbs_up_chat_emoji.toFrescoImagePath(), true, "", 0L)
                else           -> null
            }
        val message =
            when (messageType) {
        ...
        return ChatChannelMessage(sid, messageIndex, author, DateTime(timeStampAsDate.time), message, media,messageType)
    } catch (e: Exception) {
        val result =
            when (e) {
