...
//    fun getPostsForUser(@Path("userId") userId: String, @Query("page") page: Long): Single<PostsResponse>
    ...
    @GET("v1/posts?timeCutoff=&getPicture=true&populate[]=poster&populate[]=fileContent&populate[]=likes&populate[]=hosts.host&populate[]=comments.commenter&entityType=personal")
    fun getPostsForUser(@Query("entityId") userId: String, @Query("page") page: Long): Single<PostsResponse>
    ...
    @GET("v1/posts?=[populate[]=poster&populate[]=comments.commenter&populate[]=poster.schoolsAttended&populate[]=organization&populate[]=fileContent&populate[]=likes&populate[]=hosts.host&populate[]=comments.commenter")
    fun getDiscussion(@Query("entityType") entityType: String, @Query("page") page: Long, @Query("entityId") entityId: String): Single<PostsResponse>
}

class StatusCreationRequest(val poster: JsonObject,
...
class AnnouncementCreationRequest(val poster: JsonObject,
                                  val isPinned: Boolean,
                                  val isLocked: Boolean,
                                  val textContent: String,
                                  val hosts: List<Host>,
