...
class ChatDetailsInteractor @Inject constructor(@Named("chatDescription") private val chatDescription: ChatChannelDescription,
    ...
    override fun sendOtherMessages(type: String) {
            ...
            currentUser?.run {
                sendMessageInternal(
                        ChatMessage.TextMessage(UUID.randomUUID().toString(),
                                ...
                                "",
                                emptyList(),
                                ChatMyMessageState.PENDING,type),
                        null
                )
            }
        }
    ...
    override fun attachPicture(fileResource: FileResource): Single<ServerFile>{
    ...
    override fun sendPictureMessage(url: String, id: String) {
        synchronized(pendingMessages) {
            currentUser?.run {
                sendMessageInternal(
                        ChatMessage.AttachmentMessage(UUID.randomUUID().toString(),
                                ...
                                DateTime.now(),
                                "",
                                listOf(ChatAttachment.ImageURL(url)),
                                emptyList(),
                                ChatMyMessageState.PENDING),
                        null
                )
    ...
    override fun sendFileMessage(fileResource: FileResource, mediaType: String) {
        synchronized(pendingMessages) {
            currentUser?.run {
                sendMessageInternal(
                        ChatMessage.AttachmentMessage(UUID.randomUUID().toString(),
                                ...
                                if (message is ChatMessage.AttachmentMessage) {
                                    message.attachments.mapNotNull {
                                        when (it) {
                                            ...
                                            is ChatAttachment.ImageURL  -> it.path
                                            else                         -> null
                                        }
                                    }.forEach { /*it.cleanUp()*/ }
                                }
    ...
    private fun executeMessageSending(message: ChatMessage): Single<ChatMessage> {
        val sendingSingle =
                when (message) {
                    is ChatMessage.TextMessage       -> chatRepository.sendChatMessage(chatDescription.uid, message.message,null,message.messageCode)
                    is ChatMessage.AttachmentMessage -> chatRepository.sendChatMessage(chatDescription.uid,
                            ...
                            message.attachments.mapNotNull {
                                ...
                                /*is ChatAttachment.LocalImage -> it.imageFile
                                is ChatAttachment.LocalFile  -> it.fileFile*/
                                    is ChatAttachment.ImageURL  -> it.url
                                    else                         -> null
                                }
                            })
                }
